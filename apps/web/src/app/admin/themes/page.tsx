...
export default function AdminThemesPage() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async (input: { id: string; status: "upcoming" | "active" | "frozen" | "archived" }) => {
  ...
  const ThemeForm = () => {
    ...
    const { Form } = useFormedible({
      ...
      fields: [
        ...
        { name: "visibility", type: "select", label: "Visibility", options: [
        ...
        { name: "systemPrompt", type: "textarea", label: "System Prompt", textareaConfig: { rows: 4 } },
      ],
      formOptions: {
        defaultValues: editingTheme ? {
          ...
          startDate: editingTheme.startDate ? new Date(editingTheme.startDate) : null,
          endDate: editingTheme.endDate ? new Date(editingTheme.endDate) : null,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button className="gap-2" onClick={() => setEditingTheme({
          id: "",
          title: "",
          description: "",
          status: "upcoming",
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 text-sm">
                          ...
                          {theme.startDate ? new Date(theme.startDate).toLocaleDateString() : "Not set"}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <Badge variant="outline" className="capitalize">
                          {theme.visibility.replace(/_/g, " ")}
                        </Badge>
                      </td>
      ...
      <Dialog open={!!editingTheme} onOpenChange={(open) => !open && setEditingTheme(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingTheme?.id ? "Update theme configuration" : "Create a new competition theme"}
            </DialogDescription>
          </DialogHeader>
          <ThemeForm />
        </DialogContent>
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setDeleteDialog({ open: false, theme: null })}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Theme</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete "{deleteDialog.theme?.title}"? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
