...
│  │  ┌──────────┐  ┌──────────┐  ┌──────────────────┐  │   │
│  │  │ streamText│→ │  Shiki   │→ │  Sanitization   │  │   │
│  │  │  (Multi- │  │ Highlight│  │  (DOMPurify)    │  │   │
│  │  │  Provider)│  │          │  │                  │  │   │
│  │  └──────────┘  └──────────┘  └──────────────────┘  │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────┬────────────────────────────────────┘
...
On completion: sanitize + upload to CDN →
...
(function() {
  ...
  console.log('[ArcadeVibe SDK] Initialized. Use ArcadeVibe.reportScore(score) to submit scores.');
})();
```
...
export const gameSdkRouter = router({
  ...
  score: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await redis.publish(`game_scores:${input.gameId}`, JSON.stringify({
        score: input.score,
        userId: session.userId
      }));
...
export const gameScores = pgTable('game_scores', {
  ...
  idx_game_score: index('idx_game_scores_game_score').on(table.gameId, table.score.desc()),
  idx_game_user: index('idx_game_scores_game_user').on(table.gameId, table.userId),
}));
```
...
export const gameLeaderboardRouter = router({
  ...
  subscribe: publicProcedure
    ...
    .subscription(async function* ({ input }) {
      const subscriber = redis.subscribe(`game_scores:${input.gameId}`);
...
${prompt.content}`;
      ...
      const sanitized = sanitizeGameCode(fullCode);
      const assetUrl = await uploadToCDN(sanitized, game[0].id);
...
export const subscriptionPlans = pgTable('subscription_plans', {
  ...
  extraCreditPriceUsd: integer('extra_credit_price_usd').notNull(), // Price per extra credit (in cents)
...
export const indexDefinitions = {
  ...
  // Model queries
  idx_model_config_active: index('idx_model_config_active').on(modelConfig.isActive, modelConfig.tier),
};
```
...
export const adminRouter = router({
  ...
  addModel: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const model = await db.insert(modelConfig).values({
        ...input,
        isActive: true
      }).returning();
...
|------|-------|---------------|-------------------|----------|
| **Starter** | $8/mo | 50 | $0.20/credit | Basic access, all models |
| **Pro** | $18/mo | 150 | $0.15/credit | Priority queue, analytics |
| **Team** | $35/mo | 400 | $0.12/credit | Shared workspace, team analytics |
...
*Exact costs set by admin to reflect actual provider pricing.*

### Credit Purchase (Extra Credits)

Users can purchase additional credits at their plan's extra credit rate:
- Starter: $0.20/credit (min purchase: 25 credits / $5)
- Pro: $0.15/credit (min purchase: 50 credits / $7.50)
- Team: $0.12/credit (min purchase: 100 credits / $12)
