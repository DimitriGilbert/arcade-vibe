...
} from "@arcade-vibe/api/lib/script-sanitizer";
import {
...
export const generateRouter = router({
  streamGeneration: protectedProcedure
    ...
    .mutation(async function* ({ input, ctx }) {
      ...
      // Fetch allowed library patterns for theme
      ...
      const themePatterns = await db.query.themeAllowedPatterns.findMany({
        where: eq(themeAllowedPatterns.themeId, prompt.themeId),
      });
      ...
      const additionalPatterns =
        themePatternIds.length > 0
          ...
          : [];
      const allAllowedPatterns = [...globalPatterns, ...additionalPatterns];
